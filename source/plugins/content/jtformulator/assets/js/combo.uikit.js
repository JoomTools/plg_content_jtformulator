...
(function($,document,undefined)
{
	var combobox = function(options, elem)
	{
		...
		init = function(options, elem)
		{
			...
			self.$dropBtn = $(elem).find('[type="button"]');
			self.$dropBtnDiv = $(elem).find('div.uk-button-group');
			self.$dropDown = self.$dropBtnDiv.find('ul.uk-nav-dropdown');
		...
		render = function()
		{
			...
			self.$dropDown.css('left','0px');
			self.$dropDown.css('max-height','150px');
			self.$dropDown.css('overflow-y','scroll');
			//self.$dropDown.css('left',dropDownLeft+'px');
			self.$dropDown.css('left','0px');
			self.$dropBtnDiv.attr('aria-haspopup','true');
			self.$dropBtnDiv.attr('aria-expanded','false');
		},
		...
		drop = function()
		{
			if(!self.$dropDown.isEmpty)
			{
				...
				self.$dropBtnDiv.addClass('uk-open');
				self.$dropBtnDiv.attr('aria-expanded','true');
		...
		pick = function()
		{
			...
			self.$dropBtnDiv.removeClass('uk-open');
			self.$dropBtnDiv.attr('aria-expanded','false');
