...
class plgContentJtformulator extends JPlugin
{
	...
	protected function setFrameworkFieldClass()
	{
		...
		{
			if (!empty($classes['orientation']))
			{
				$this->setFieldsetClass($fieldset, $classes['orientation']);
			}

			foreach ($fieldset as $field)
			{
				$this->setFieldClass((string)$field['name'], $classes);
			}
		}
	}

	protected function setFieldsetClass($fieldset, $classes)
	{
		$fieldsetClass = array();

		if (!empty((string) $fieldset['class']))
		{
			$fieldsetClass = array_flip(explode(' ', (string) $fieldset['class']));
		}

		$orientationClass = array_flip($classes);

		$class = array_merge($orientationClass, $fieldsetClass);

		$fieldset->addAttribute('class', implode(' ', array_keys($class)));
	}
